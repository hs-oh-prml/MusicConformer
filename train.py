...
from model.music_conformer import MusicConformer
...
def main():
    ...
    model = MusicConformer(n_layers=args.n_layers, num_heads=args.num_heads,
                d_model=args.d_model, dim_feedforward=args.dim_feedforward, dropout=args.dropout,
                max_sequence=args.max_sequence, rpr=args.rpr).to(get_device())


    num_param = sum(param.numel() for param in model.parameters())
    print("Number of Music Transformer Parameters: {}\n".format(num_param))
    ...
    for epoch in range(start_epoch, args.epochs):
        inner_bar = tqdm(total=args.epochs, desc="Epoch {}".format(epoch), position=0)
        inner_bar.update(1)
